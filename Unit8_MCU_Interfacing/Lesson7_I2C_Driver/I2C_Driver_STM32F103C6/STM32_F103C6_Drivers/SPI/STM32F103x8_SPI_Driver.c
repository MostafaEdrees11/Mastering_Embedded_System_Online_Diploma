...
SPI_Configuration_t Gl_SPI1_CFG;
SPI_Configuration_t Gl_SPI2_CFG;
...
void MCAL_SPI_Init(SPI_TypeDef *SPIx, SPI_Configuration_t *SPI_Config)
{
	...
	if(SPIx == SPI1)
	{
		Gl_SPI1_CFG = *SPI_Config;
		Gl_ASPI_CFG[SPI1_INDEX] = &Gl_SPI1_CFG;
		RCC_SPI1_CLK_EN();
	}
	else if(SPIx == SPI2)
	{
		Gl_SPI2_CFG = *SPI_Config;
		Gl_ASPI_CFG[SPI2_INDEX] = &Gl_SPI2_CFG;
		RCC_SPI2_CLK_EN();
	}
 ...
 * @retval 		-none
 * Note			-we should enable the corresponding ALT & GPIO in RCC clock also called after MCAL_SPI_Init()
================================================================**/
